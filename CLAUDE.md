...
2. 中に以下を作る：`prediction.html`（メイン）、`research.html`（情報まとめ）、`odds.html`（計算機）、`analysis/1_展開予想.html`〜`5_オッズ分析.html`、`style.css`（ルートの `style.css` をコピー）。
...
6. **データ収集の注意**：このクラウド環境はWeb検索ができない場合がある。出馬表・オッズ・馬体重などが無いと予想は組めないので、
   ...
   - ゼロからの本格予想（5視点のWeb調査含む）は、Web検索が使えるPCのClaude Codeの方が得意。スマホは「データを渡して組む／直前更新」が向く。
