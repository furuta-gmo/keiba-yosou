...
└── 2026-06-14_takarazuka/  レースごとのフォルダ（romaji名）
    ...
    ├── style.css           （ルートのコピー）
    └── analysis/1..8_*.html 8視点分析（①展開②血統③近走④コース馬場⑤オッズ⑥調教⑦指数⑧データ傾向）
```
...
2. 中に以下を作る：`prediction.html`（メイン）、`research.html`（情報まとめ）、`odds.html`（計算機）、`analysis/1_展開予想.html`〜`5_オッズ分析.html`＋`6_調教.html`・`7_指数.html`・`8_データ傾向.html`（**計8視点**）、`style.css`（ルートの `style.css` をコピー）。
   - prediction.html には8視点ぶんのnavcard・「8視点の分析」セクション・各ページのpagenav（①〜⑧）を用意する（既存の `2026-06-21_fuchu-himba/` が雛形）。
